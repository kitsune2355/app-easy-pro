...
export const useRepairForm = () => {
  ...
  const schema: yup.ObjectSchema<IRepairForm> = yup.object().shape({
    report_date: yup.string().required(t(`${invalid}.DATE`)).default(dayJs().format('YYYY-MM-DD')),
    report_time: yup.string().required(t(`${invalid}.TIME`)).default(dayJs().format('HH:mm')),
